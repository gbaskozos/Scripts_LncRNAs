...
library("doParallel")
library(iterators)
source("/home/george/Desktop/LncRNA_v2/IPS/scripts/dropDetect.R")
source("/home/george/Desktop/LncRNA_v2/IPS/scripts/BAM_to_IOE.R")
####################
# Convert Z-scores #
####################
...
PATH <- "/media/george/data3tb/Sequencing_Data/ips_neurons/merged_bams"
...
########################################
src_files <- list.files(path=PATH, pattern="*bam.sorted.bam$", full.names=TRUE)
bai_files <- list.files(path=PATH, pattern="*bam.sorted.bam.bai$", full.names=TRUE)

file_names <- list.files(path=PATH, pattern="*bam.sorted.bam$", full.names=FALSE)

IPS <- c(grep("_IPS", src_files, value=TRUE))
 
NEURONS <- c(grep("_NEURONS", src_files, value=TRUE))
 
AD2 <- c(grep("AD2_", src_files, value=TRUE))
 
AD4 <- c(grep("AD4_", src_files, value=TRUE))
 
NHDF <- c(grep("NHDF_", src_files, value=TRUE))

design_all <- c(IPS, NEURONS)

bam_names <- c(IPS, NEURONS)

bamlist <- BamFileList(c(IPS, NEURONS), index = c(IPS, NEURONS), yieldSize = 1000000)

bam_names <- substr(bam_names, 63, nchar(bam_names)-22)

################################################
##     DOWNLOAD KNOWN GENE SET ANNOTATIONS    ## 
##Create txdb objects, group features by genes## 
################################################
if (!file.exists(file = paste(PATH_results, "/HS_genome/iGRanges.RData", sep=""))) {

if (!file.exists(file = paste(PATH_results,"/HS_genome/ENSEMBL_HS_exonsByGene.RData", sep=""))) {
...
IPS <- useMart("ensembl",dataset="hsapiens_gene_ensembl")

ens_annot <- getBM(attributes=c('ensembl_transcript_id', 'description', 'transcript_biotype'), filters = 'ensembl_transcript_id', values = as.character(transcripts(txdb)$tx_name), mart = IPS)
...
write.csv(file = paste(PATH_results,"/HS_genome/ensembl_annot.csv", sep=""), ens_annot)

write.csv(file = paste(PATH_results,"/HS_genome/ensembl_nc.csv", sep=""), nc_ensembl)
...
export.bed(tx_by_gene, paste(PATH_results,"/HS_genome/ENSEMBL_HS_genes.bed", sep=""))
export(asGFF(tx_by_gene), paste(PATH_results,"/HS_genome/ENSEMBL_HS_genes.gtf", sep=""),  format="gtf")
...
save(file = paste(PATH_results,"/HS_genome/ENSEMBL_HS_intronsByGene.RData", sep=""), intronsByGene)
export(asGFF(intronsByGene), paste(PATH_results,"/HS_genome/intronsByGene.gtf", sep="") , format="gtf")
export.bed(intronsByGene, paste(PATH_results,"/HS_genome/intronsByGene.bed", sep=""))
####
tx_by_gene <- reduce(unlist(exonsBy(txdb,"gene")))

save(txdb, tx_by_gene, file = paste(PATH_results,"/HS_genome/ENSEMBL_HS_exonsByGene.RData", sep="") )

########################
# Export known lncRNAs #
########################
txdb_lncs <- makeTxDbFromBiomart(biomart="ENSEMBL_MART_ENSEMBL", dataset="hsapiens_gene_ensembl", transcript_ids=as.character(nc_ensembl$ensembl_transcript_id), circ_seqs=DEFAULT_CIRC_SEQS, filter=NULL, id_prefix="ensembl_", host="www.ensembl.org", port=80, taxonomyId=NA, miRBaseBuild=NA)
...
export(asGFF(tx_by_lnc), paste(PATH_results,"/HS_genome/ENSEMBL_HS_LncRNAs.gtf", sep=""),  format="gtf")
export.bed(tx_by_lnc, paste(PATH_results,"/HS_genome/ENSEMBL_HS_LncRNAs.bed", sep=""))

ens_lncs_names <- names(tx_by_lnc)
write.csv(file = paste(PATH_results,"/HS_genome/ens_lncs_names.csv", sep=""), ens_lncs_names)

ens_lncs_annot <- getBM(attributes=c('ensembl_gene_id', 'hgnc_symbol', 'description', 'gene_biotype'), filters = 'ensembl_gene_id', values = as.character(ens_lncs_names), mart = IPS)
...
export.bed(antisense_ens_lncs, paste(PATH_results,"/HS_genome/ENSEMBL_HS_antisense.bed", sep=""))
export(asGFF(antisense_ens_lncs), paste(PATH_results,"/HS_genome/ENSEMBL_HS_antisense.gtf", sep=""),  format="gtf")

export.bed(intergenic_ens_lncs, paste(PATH_results,"/HS_genome/ENSEMBL_HS_intergenic.bed", sep=""))
export(asGFF(intergenic_ens_lncs), paste(PATH_results,"/HS_genome/ENSEMBL_HS_intergenic.gtf", sep=""),  format="gtf")

write.csv(file = paste(PATH_results,"/HS_genome/ens_lncs_annotation.csv", sep=""), ens_lncs_annot)

save(tx_by_lnc, nc_ensembl, txdb_lncs, ens_lncs_annot, ens_lncs_annot, ens_lncs_names, file = paste(PATH_results,"/HS_genome/ENSEMBL_HS_lncRNAs.RData", sep="")) 
}

#######################################################################

load(paste(PATH_results,"/HS_genome/ENSEMBL_HS_exonsByGene.RData", sep=""))
load(paste(PATH_results,"/HS_genome/ENSEMBL_HS_intronsByGene.RData", sep=""))

if (!file.exists(file = paste(PATH_results,"/HS_genome/Refseq_HS_exonsByGene.RData", sep=""))) {

txdb_RG <- makeTxDbFromUCSC(genome='hg38',tablename='ncbiRefSeq')
...
export.bed(tx_RG_by_gene,paste(PATH_results,"/HS_genome/HS_refseq.bed", sep=""))

save(tx_RG_by_gene, file = paste(PATH_results,"/HS_genome/Refseq_HS_exonsByGene.RData", sep=""))
...
load(paste(PATH_results, "/HS_genome/Refseq_HS_exonsByGene.RData", sep=""))

txdb_xRG <- makeTxDbFromUCSC(genome='hg38',tablename='xenoRefGene')

xenoRefGenes <- reduce(unlist(exonsBy(txdb_xRG, "tx")))

#xenoRefGenes <- import.gff(paste(PATH_results, "/HS_genome/xenoRefGene.gtf", sep=""))
...
save(xenoRefGenes, file = paste(PATH_results, "/HS_genome/xenoRefGenes.RData", sep=""))
...
save(gRanges, file = paste(PATH_results, "/HS_genome/HS_gRanges.RData", sep=""))

################################################################
##Extend genes by 1000 bp to avoid UTRs or non annotated exons##
################################################################
load(paste(PATH_results, "/HS_genome/HS_gRanges.RData", sep=""))
gRangesExt <- gRanges
...
save(gRangesExt, file = paste(PATH_results, "/HS_genome/HS_gRangesExt.RData", sep=""))

#export.bed(gRangesExt, paste(PATH_results, "/HS_genome/HS_gRangesExt.bed", sep=""))
...
save(igRangesExt, file = paste(PATH_results, "/HS_genome/iGRanges.RData", sep=""))

#export.bed(igRangesExt, paste(PATH_results, "/HS_genome/iGRanges.bed", sep=""))
}

load(paste(PATH_results, "/HS_genome/iGRanges.RData", sep=""))
...
bplapply(bamlist, FUN = BAM_to_IOE, BPPARAM = MulticoreParam(), PATH=PATH, PATH_results=PATH_results, igRangesExt=igRangesExt, param = param, len=100, dep=2, suffix=22)
...
#Remove outlying transcripts with > 46 regions
rRegs_complete <- rRegs_complete[unlist(mclapply(rRegs_complete, function(x) length(x), mc.cores=n_cores, mc.cleanup=TRUE, mc.allow.recursive=TRUE)) <=46 ] 
...
# Load count files #
####################
count_files <- list.files(path=PATH, pattern="*.lnc_complete.counts.tab$", full.names=TRUE)
IPS <- c(grep("_IPS", count_files, value=TRUE))
NEURONS <- c(grep("_NEURONS", count_files, value=TRUE))
AD2 <- c(grep("AD2_", count_files, value=TRUE))
AD4 <- c(grep("AD4_", count_files, value=TRUE))
NHDF <- c(grep("NHDF_", count_files, value=TRUE))
count_files_ord <- c(IPS, NEURONS)
...
export.bed(rRegs_complete_coff_nc, paste(PATH_results,"/HS_genome/rRegs_complete_coff_nc.bed", sep=""))
export(asGFF(rRegs_complete_coff_nc), paste(PATH_results,"/myRegs_cov/rRegs_complete_coff_nc.gtf", sep=""), format="gtf")

export.bed(rRegs_complete_coff_coding, paste(PATH_results,"/HS_genome/rRegs_complete_coff_coding.bed", sep=""))
