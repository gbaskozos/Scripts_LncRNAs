...
lincs_closest_ens$gene_symbol <- mapIds(org.Hs.eg.db, keys=as.character(lincs_closest_ens$ENSEMBL_gene_id), column="SYMBOL", keytype="ENSEMBL", multiVals="first")

lincs_closest_ens$LincRNA_symbol <- mapIds(org.Hs.eg.db, keys=as.character(lincs_closest_ens$lincRNA), column="SYMBOL", keytype="ENSEMBL", multiVals="first")

lincs_closest_ens <- lincs_closest_ens[as.character(lincs_closest_ens$ENSEMBL_gene_id) %in% rownames(rld_mat),]

lincs_closest_ens$baseMean <- AD2[as.character(lincs_closest_ens$lincRNA),]$baseMean

lincs_closest_ens$lnc_lfc_AD2 <- AD2[as.character(lincs_closest_ens$lincRNA),]$log2FoldChange

lincs_closest_ens$lnc_pvalue_AD2 <- AD2[as.character(lincs_closest_ens$lincRNA),]$padj

lincs_closest_ens$gene_lfc_AD2 <- AD2[as.character(lincs_closest_ens$ENSEMBL_gene_id),]$log2FoldChange

lincs_closest_ens$gene_pvalue_AD2 <- AD2[as.character(lincs_closest_ens$ENSEMBL_gene_id),]$padj


lincs_closest_ens$lnc_lfc_AD4 <- AD4[as.character(lincs_closest_ens$lincRNA),]$log2FoldChange

lincs_closest_ens$lnc_pvalue_AD4 <- AD4[as.character(lincs_closest_ens$lincRNA),]$padj

lincs_closest_ens$gene_lfc_AD4 <- AD4[as.character(lincs_closest_ens$ENSEMBL_gene_id),]$log2FoldChange

lincs_closest_ens$gene_pvalue_AD4 <- AD4[as.character(lincs_closest_ens$ENSEMBL_gene_id),]$padj


lincs_closest_ens$lnc_lfc_NHDF <- NHDF[as.character(lincs_closest_ens$lincRNA),]$log2FoldChange

lincs_closest_ens$lnc_pvalue_NHDF <- NHDF[as.character(lincs_closest_ens$lincRNA),]$padj

lincs_closest_ens$gene_lfc_NHDF <- NHDF[as.character(lincs_closest_ens$ENSEMBL_gene_id),]$log2FoldChange

lincs_closest_ens$gene_pvalue_NHDF <- NHDF[as.character(lincs_closest_ens$ENSEMBL_gene_id),]$padj
...
names(lincs_closest_ens) <- c(names(lincs_closest_ens)[1:18], colnames(rld_mat_lincs_ens), colnames(rld_mat_genes))

for (i in 1:nrow(lincs_closest_ens)) {
lincs_closest_ens$cor[i] <- cor.test(x=as.numeric(lincs_closest_ens[i,c(19:30)]), y=as.numeric(lincs_closest_ens[i,c(31:42)]))$estimate  

lincs_closest_ens$cor_pvalue[i] <- cor.test(x=as.numeric(lincs_closest_ens[i,c(19:30)]), y=as.numeric(lincs_closest_ens[i,c(31:42)]))$p.value 
}
...
##############
AD2_sig_ens <- lincs_closest_ens[lincs_closest_ens$lnc_pvalue_AD2 < 0.05 & !is.na(lincs_closest_ens$lnc_pvalue_AD2),]

AD4_sig_ens <- lincs_closest_ens[lincs_closest_ens$lnc_pvalue_AD4 < 0.05 & !is.na(lincs_closest_ens$lnc_pvalue_AD4),]

NHDF_sig_ens <- lincs_closest_ens[lincs_closest_ens$lnc_pvalue_NHDF < 0.05 & !is.na(lincs_closest_ens$lnc_pvalue_NHDF),]



pdf(paste (PATH, "/Distance_ensembl_lincRNAs.pdf", sep= ""))
boxplot(abs(lincs_closest_ens$Distance), abs(sig_true$Distance), abs(AD2_sig_ens$Distance), abs(AD4_sig_ens$Distance), abs(NHDF_sig_ens$Distance),  pars=list(ylim=c(0,1000000)), names=c("All LincRNAs", "Sig. correlated", "DE in AD2", "DE in AD4", "DE in NHDF"), main = "Distance between ENSEMBL genes and LincRNAs" , cex.axis=0.5, las=1)
...
AD2_sig_pain <- lincs_pain[lincs_pain$lnc_pvalue_AD2 < 0.05 & !is.na(lincs_pain$lnc_pvalue_AD2) & lincs_pain$gene_pvalue_AD2 < 0.05,]

AD4_sig_pain <- lincs_pain[lincs_pain$lnc_pvalue_AD4 < 0.05 & !is.na(lincs_pain$lnc_pvalue_AD4) & lincs_pain$gene_pvalue_AD4 < 0.05,]

NHDF_sig_pain <- lincs_pain[lincs_pain$lnc_pvalue_NHDF < 0.05 & !is.na(lincs_pain$lnc_pvalue_NHDF) & lincs_pain$gene_pvalue_NHDF < 0.05,]
...
write.csv(file = paste(PATH, "/DE/lincs_ENSEMBL_AD2_sig.csv", sep=""), AD2_sig_ens[order(-abs(AD2_sig_ens$lnc_lfc_AD2), -AD2_sig_ens$baseMean),])

write.csv(file = paste(PATH, "/DE/lincs_ENSEMBL_AD4_sig.csv", sep=""), AD4_sig_ens)

write.csv(file = paste(PATH, "/DE/lincs_ENSEMBL_NHDF_sig.csv", sep=""), NHDF_sig_ens)
...
toc_lncs <- read.csv(file = paste(PATH,"/LncRNAs_toc_IPS_DRG.csv", sep=""), row.name=1)
...
lincs_closest <- data.frame(lincRNA = lincs_closest$lincRNA, ENSEMBL_gene_id = lincs_closest$V16, Distance = lincs_closest$V25, gene_symbol = mapIds(org.Hs.eg.db, keys=as.character(lincs_closest$V16), column="SYMBOL", keytype="ENSEMBL", multiVals="first"), LincRNA_symbol = lincs_closest$V4)
...
lincs_closest$baseMean <- AD2[as.character(lincs_closest$lincRNA),]$baseMean


lincs_closest$lnc_lfc_AD2 <- AD2[as.character(lincs_closest$lincRNA),]$log2FoldChange

lincs_closest$lnc_pvalue_AD2 <- AD2[as.character(lincs_closest$lincRNA),]$padj

lincs_closest$gene_lfc_AD2 <- AD2[as.character(lincs_closest$ENSEMBL_gene_id),]$log2FoldChange

lincs_closest$gene_pvalue_AD2 <- AD2[as.character(lincs_closest$ENSEMBL_gene_id),]$padj


lincs_closest$lnc_lfc_AD4 <- AD4[as.character(lincs_closest$lincRNA),]$log2FoldChange

lincs_closest$lnc_pvalue_AD4 <- AD4[as.character(lincs_closest$lincRNA),]$padj

lincs_closest$gene_lfc_AD4 <- AD4[as.character(lincs_closest$ENSEMBL_gene_id),]$log2FoldChange

lincs_closest$gene_pvalue_AD4 <- AD4[as.character(lincs_closest$ENSEMBL_gene_id),]$padj


lincs_closest$lnc_lfc_NHDF <- NHDF[as.character(lincs_closest$lincRNA),]$log2FoldChange

lincs_closest$lnc_pvalue_NHDF <- NHDF[as.character(lincs_closest$lincRNA),]$padj

lincs_closest$gene_lfc_NHDF <- NHDF[as.character(lincs_closest$ENSEMBL_gene_id),]$log2FoldChange

lincs_closest$gene_pvalue_NHDF <- NHDF[as.character(lincs_closest$ENSEMBL_gene_id),]$padj
...
names(lincs_closest) <- c(names(lincs_closest)[1:18], colnames(rld_mat_lincs), colnames(rld_mat_genes))




for (i in 1:nrow(lincs_closest)) {
lincs_closest$cor[i] <- cor.test(x=as.numeric(lincs_closest[i,c(19:30)]), y=as.numeric(lincs_closest[i,c(31:42)]))$estimate  

lincs_closest$cor_pvalue[i] <- cor.test(x=as.numeric(lincs_closest[i,c(19:30)]), y=as.numeric(lincs_closest[i,c(31:42)]))$p.value 
}
...
##############
AD2_sig <- lincs_closest[lincs_closest$lnc_pvalue_AD2 < 0.05 & !is.na(lincs_closest$lnc_pvalue_AD2),]

AD4_sig <- lincs_closest[lincs_closest$lnc_pvalue_AD4 < 0.05 & !is.na(lincs_closest$lnc_pvalue_AD4),]

NHDF_sig <- lincs_closest[lincs_closest$lnc_pvalue_NHDF < 0.05 & !is.na(lincs_closest$lnc_pvalue_NHDF),]


pdf(paste (PATH, "/Distance_novel_lincRNAs.pdf", sep= ""))
boxplot(abs(lincs_closest$Distance), abs(sig_true$Distance), abs(AD2_sig$Distance), abs(AD4_sig$Distance), abs(NHDF_sig$Distance),  pars=list(ylim=c(0,1000000)), names=c("All LincRNAs", "Sig. correlated", "DE in AD2", "DE in AD4", "DE in NHDF"), main = "Distance between ENSEMBL genes and LincRNAs" , cex.axis=0.5, las=1)
...
write.csv(file = paste(PATH, "/DE/lincs_AD2_sig.csv", sep=""), AD2_sig)

write.csv(file = paste(PATH, "/DE/lincs_AD4_sig.csv", sep=""), AD4_sig)

write.csv(file = paste(PATH, "/DE/lincs_NHDF_sig.csv", sep=""), NHDF_sig)
