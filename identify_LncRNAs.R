...
library("doParallel")
library(iterators)
source("$HOME/Desktop/LncRNA_v2/Mouse/scripts/dropDetect.R")
source("$HOME/Desktop/LncRNA_v2/Mouse/scripts/BAM_to_IOE.R")
####################
# Convert Z-scores #
####################
...
PATH <- "/media/george/data3tb/Sequencing_Data/Mouse_DRG_sni_jmogil/merged_bams"
...
########################################
src_files <- list.files(path=PATH, pattern="*.sorted.bam$", full.names=TRUE)
bai_files <- list.files(path=PATH, pattern="*.sorted.bam.bai$", full.names=TRUE)

file_names <- list.files(path=PATH, pattern="*.bam$", full.names=FALSE)


sham <- c(grep("SHAM", src_files, value=TRUE))

sni <- c(grep("SNI", src_files, value=TRUE))

BALBc <- c(grep("BALB.c", src_files, value=TRUE))

B10D2 <- c(grep("B10.D2", src_files, value=TRUE))

MALE <- c(grep("M.bam", src_files, value=TRUE))

FEMALE <- c(grep("F.bam", src_files, value=TRUE))

sham_names <- c(grep("SHAM", file_names, value=TRUE))
sni_names <- c(grep("SNI", file_names, value=TRUE))

#bamlist <- BamFileList(c(sham, sni), index = c(sham, sni), yieldSize = 9000000)

bamlist <- BamFileList(c(sham, sni), index = c(sham, sni), yieldSize = 1000000)


#seqlevels(bamlist, pruning.mode="coarse") <- seqlevels(bamlist)[grepl("^[0-9, X, Y]*$", seqlevels(bamlist))]

bam_names <- c(sham_names, sni_names)

bam_names <- substr(bam_names, 0, nchar(bam_names)-11)

################################################
##     DOWNLOAD KNOWN GENE SET ANNOTATIONS    ## 
##Create txdb objects, group features by genes## 
################################################
if (!file.exists(file = paste(PATH_results, "/mm10_genome/iGRanges.RData", sep=""))) {

if (!file.exists(file = paste(PATH_results,"/mm10_genome/ENSEMBL_mm10_exonsByGene.RData", sep=""))) {

txdb <- makeTxDbFromBiomart(biomart="ENSEMBL_MART_ENSEMBL", dataset="mmusculus_gene_ensembl", transcript_ids=NULL, circ_seqs=DEFAULT_CIRC_SEQS, filter=NULL, id_prefix="ensembl_", host="www.ensembl.org", port=80, taxonomyId=NA, miRBaseBuild=NA)

mouse <- useMart("ensembl",dataset="mmusculus_gene_ensembl")

ens_annot <- getBM(attributes=c('ensembl_transcript_id', 'description', 'transcript_biotype'), filters = 'ensembl_transcript_id', values = as.character(transcripts(txdb)$tx_name), mart = mouse)
...
write.csv(file = paste(PATH_results,"/mm10_genome/ensembl_annot.csv", sep=""), ens_annot)

write.csv(file = paste(PATH_results,"/mm10_genome/ensembl_nc.csv", sep=""), nc_ensembl)
...
export.bed(tx_by_gene, paste(PATH_results,"/mm10_genome/ENSEMBL_mm10_genes.bed", sep=""))
export(asGFF(tx_by_gene), paste(PATH_results,"/mm10_genome/ENSEMBL_mm10_genes.gtf", sep=""),  format="gtf")
...
save(file = paste(PATH_results,"/mm10_genome/ENSEMBL_mm10_intronsByGene.RData", sep=""), intronsByGene)
export(asGFF(intronsByGene), paste(PATH_results,"/mm10_genome/intronsByGene.gtf", sep="") , format="gtf")
export.bed(intronsByGene, paste(PATH_results,"/mm10_genome/intronsByGene.bed", sep=""))
####
tx_by_gene <- reduce(unlist(exonsBy(txdb,"gene")))

save(txdb, tx_by_gene, file = paste(PATH_results,"/mm10_genome/ENSEMBL_mm10_exonsByGene.RData", sep="") )

########################
# Export known lncRNAs #
########################
txdb_lncs <- makeTxDbFromBiomart(biomart="ENSEMBL_MART_ENSEMBL", dataset="mmusculus_gene_ensembl", transcript_ids=as.character(nc_ensembl$ensembl_transcript_id), circ_seqs=DEFAULT_CIRC_SEQS, filter=NULL, id_prefix="ensembl_", host="www.ensembl.org", port=80, taxonomyId=NA, miRBaseBuild=NA)
...
export(asGFF(tx_by_lnc), paste(PATH_results,"/mm10_genome/ENSEMBL_mm10_LncRNAs.gtf", sep=""),  format="gtf")
export.bed(tx_by_lnc, paste(PATH_results,"/mm10_genome/ENSEMBL_mm10_LncRNAs.bed", sep=""))

ens_lncs_names <- names(tx_by_lnc)
write.csv(file = paste(PATH_results,"/mm10_genome/ens_lncs_names.csv", sep=""), ens_lncs_names)

ens_lncs_annot <- getBM(attributes=c('ensembl_gene_id', 'mgi_symbol', 'description', 'gene_biotype'), filters = 'ensembl_gene_id', values = as.character(ens_lncs_names), mart = mouse)
...
export.bed(antisense_ens_lncs, paste(PATH_results,"/mm10_genome/ENSEMBL_mm10_antisense.bed", sep=""))
export(asGFF(antisense_ens_lncs), paste(PATH_results,"/mm10_genome/ENSEMBL_mm10_antisense.gtf", sep=""),  format="gtf")

export.bed(intergenic_ens_lncs, paste(PATH_results,"/mm10_genome/ENSEMBL_mm10_intergenic.bed", sep=""))
export(asGFF(intergenic_ens_lncs), paste(PATH_results,"/mm10_genome/ENSEMBL_mm10_intergenic.gtf", sep=""),  format="gtf")

write.csv(file = paste(PATH_results,"/mm10_genome/ens_lncs_annotation.csv", sep=""), ens_lncs_annot)

save(tx_by_lnc, nc_ensembl, txdb_lncs, ens_lncs_annot, ens_lncs_annot, ens_lncs_names, file = paste(PATH_results,"/mm10_genome/ENSEMBL_mm10_lncRNAs.RData", sep="")) 
}

#######################################################################

load(paste(PATH_results,"/mm10_genome/ENSEMBL_mm10_exonsByGene.RData", sep=""))
load(paste(PATH_results,"/mm10_genome/ENSEMBL_mm10_intronsByGene.RData", sep=""))

if (!file.exists(file = paste(PATH_results,"/mm10_genome/Refseq_mm10_exonsByGene.RData", sep=""))) {

txdb_RG <- makeTxDbFromUCSC(genome='mm10',tablename='ncbiRefSeq')

tx_RG_by_gene <- reduce(unlist(exonsBy(txdb_RG, "tx")))
...
export.bed(tx_RG_by_gene,paste(PATH_results,"/mm10_genome/mm10_refseq.bed", sep=""))

save(tx_RG_by_gene, file = paste(PATH_results,"/mm10_genome/Refseq_mm10_exonsByGene.RData", sep=""))
...
load(paste(PATH_results, "/mm10_genome/Refseq_mm10_exonsByGene.RData", sep=""))

txdb_xRG <- makeTxDbFromUCSC(genome='mm10',tablename='xenoRefGene')

xenoRefGenes <- reduce(unlist(exonsBy(txdb_xRG, "tx")))

#xenoRefGenes <- import.gff(paste(PATH_results, "/mm10_genome/xenoRefGene.gtf", sep=""))
...
save(xenoRefGenes, file = paste(PATH_results, "/mm10_genome/xenoRefGenes.RData", sep=""))
...
save(gRanges, file = paste(PATH_results, "/mm10_genome/mm10_gRanges.RData", sep=""))

################################################################
##Extend genes by 1000 bp to avoid UTRs or non annotated exons##
################################################################
load(paste(PATH_results, "/mm10_genome/mm10_gRanges.RData", sep=""))
gRangesExt <- gRanges
...
save(gRangesExt, file = paste(PATH_results, "/mm10_genome/mm10_gRangesExt.RData", sep=""))

#export.bed(gRangesExt, paste(PATH_results, "/mm10_genome/mm10_gRangesExt.bed", sep=""))
...
save(igRangesExt, file = paste(PATH_results, "/mm10_genome/iGRanges.RData", sep=""))

#export.bed(igRangesExt, paste(PATH_results, "/mm10_genome/iGRanges.bed", sep=""))
}

load(paste(PATH_results, "/mm10_genome/iGRanges.RData", sep=""))
...
bplapply(bamlist, FUN = BAM_to_IOE, BPPARAM = MulticoreParam(), PATH=PATH, PATH_results=PATH_results, igRangesExt=igRangesExt, param = param, len=100, dep=2, suffix=11)
...
rRegs_complete <- rRegs_complete[unlist(mclapply(rRegs_complete, function(x) length(x), mc.cores=n_cores, mc.cleanup=TRUE, mc.allow.recursive=TRUE)) <=42 ] 
...
sham_counts <- c(grep("SHAM", count_files, value=TRUE))
sni_counts <- c(grep("SNI", count_files, value=TRUE))
balbc_counts <- c(grep("BALB.c", count_files, value=TRUE))
b10d2_couns <- c(grep("B10.D2", count_files, value=TRUE))
count_files_ord <- c(sham_counts, sni_counts)
...
cpat_out <- rbind(cpat_1, cpat_2, cpat_3, cpat_4, cpat_5, cpat_6)
...
export.bed(rRegs_complete_coff_nc, paste(PATH_results,"/mm10_genome/rRegs_complete_coff_nc.bed", sep=""))
export(asGFF(rRegs_complete_coff_nc), paste(PATH_results,"/myRegs_cov/rRegs_complete_coff_nc.gtf", sep=""), format="gtf")

export.bed(rRegs_complete_coff_coding, paste(PATH_results,"/mm10_genome/rRegs_complete_coff_coding.bed", sep=""))
...
source("./DE_and_WGCNA.R")

#Annotate antisense LncRNAs
source("./annotate_expression.R", echo=TRUE)

#Annotate intergenic LncRNAs
source("./closest_gene_lincs.R", echo=TRUE)

#Analyse TSS
source("./calc_start_sites.R", echo=TRUE)

#Antisense of orthologs and in syntenic blocks
system("./synteny.sh")
source("./homologs.R", echo=TRUE)
