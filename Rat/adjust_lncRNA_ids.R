...
lncRNAs_bed <-  read.table(paste(PATH_results,"/rn6_genome/rRegs_complete_coff_nc.bed", sep=""), sep= "\t")

lncRNAs_bed <- lncRNAs_bed[as.character(lncRNAs_bed$V4) %in% lnc_names,]

write.table(lncRNAs_bed, file = paste(PATH_results,"/rn6_genome/Novel_lncRNAs_rat_DRG.bed", sep=""), sep="\t", quote=FALSE, row.name=FALSE, col.names=FALSE)

write.table(lncRNAs, file = paste(PATH_results,"/myRegs_cov/Novel_lncRNAs_rat_DRG.gtf", sep=""), sep="\t", quote=FALSE, row.name=FALSE, col.names=FALSE)

rownames(toc_coff_nc) <- gsub("mRNA", "LncRNA", rownames(toc_coff_nc))

LncRNAs_toc <- toc_coff_nc[rownames(toc_coff_nc) %in% new_ids,]

write.csv(LncRNAs_toc, file = paste(PATH_results,"/LncRNAs_toc_rat_DRG.csv", sep=""))

save(LncRNAs_toc, file = paste(PATH_results,"/LncRNAs_toc_rat_DRG.RData", sep=""))
